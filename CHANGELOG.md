...
### Added
- Added tox and GitHub Actions CI coverage for Python 3.12 and 3.13.
- Added installed-package smoke checks for `ytt --help`, `ytt --version`, and `import ytt`.
- Documented the required `main` branch protection policy.

### Fixed
- Removed the stale top-level `ytt/` package so checkout imports no longer shadow `src/ytt`.
...
### Fixed
- Parse Markdown-formatted links whose link text contains escaped brackets (for example `\[Full Episode\]`) so pasted YouTube links reliably extract the video ID.

## [0.9.1] - 2026-02-14

### Added
- Allow URL-less fetch flag invocations (for example `ytt fetch --refresh` and `ytt --refresh`) to derive the YouTube URL from clipboard instead of failing on missing `youtube_url`.
